...
private:
    ...
    int registerNameNum = 0;

public:
    /**
     * Sent a name register notification signals
     */
    class nameRegister : public cObject
    {
      public:
        iName name;
        iName originalRouter;

      public:
        nameRegister(iName registerName, iName routerName)
            : name(registerName), originalRouter(routerName){}
    };

    /** @brief Signals used to publish name register. */
    static const simsignal_t nameRegisterTriggerSignal;
...
public:
    ...
    void setMtu(unsigned mtus) { mtu = mtus;}
    // return interest and create block
    shared_ptr<contentBlock> getBlock(const iName &interest);
    shared_ptr<contentBlock> createBlock(const iName &interest, double lifeTime);

    void cachePacket(const iName &interestName, Packet *packet, iName & originRouter);
    void cachePacketFromHL(const iName &interestName, Packet *packet, iName & originRouter);
