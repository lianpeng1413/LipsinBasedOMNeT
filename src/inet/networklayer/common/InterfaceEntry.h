...
class INET_API InterfaceEntry : public cModule
{
  ...
  protected:
    ...
    std::map<long int,simtime_t> nonceTable;
  ...
  public:
    ...
    /**
     * Add functions of Noncetable for Lipsin to detect the routing loop
     */
    bool setDeletePeriod(simtime_t dp){ deletePeriod = dp; return true; }
    bool findNonce(long int n){
        if(nonceTable.find(n) == nonceTable.end()) return false;
        if(simTime() > nonceTable[n] + deletePeriod){
            nonceTable.erase(n);
            return false;
        }else return true;
    }
    bool insertNonce(long int n){
        nonceTable[n] = simTime();
        return true;
    }
