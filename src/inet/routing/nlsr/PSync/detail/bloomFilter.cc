...
BloomParameters::BloomParameters() : minimum_size(1), maximum_size(std::numeric_limits<unsigned int>::max())
    , minimum_number_of_hashes(1), maximum_number_of_hashes(std::numeric_limits<unsigned int>::max())
    , projected_element_count(200), false_positive_probability(1.0 / projected_element_count)
...
BloomFilter::BloomFilter(): bit_table_(0), salt_count_(0) , table_size_(0), raw_table_size_(0), projected_element_count_(0)
    , inserted_element_count_(0), random_seed_(0), desired_false_positive_probability_(0.0){}

BloomFilter::BloomFilter(const BloomParameters& p): bit_table_(0), projected_element_count_(p.projected_element_count)
    , inserted_element_count_(0), random_seed_((p.random_seed * 0xA5A5A5A5) + 1)
...
BloomFilter::BloomFilter(unsigned int projected_element_count, double false_positive_probability)
    : BloomFilter(getParameters(projected_element_count, false_positive_probability))
{
}

BloomFilter::BloomFilter(unsigned int projected_element_count, double false_positive_probability, const iName& bfName)
     : BloomFilter(projected_element_count, false_positive_probability)
{
    std::vector<BloomFilter::cell_type> table(bfName.str().size());

    if (table.size() != raw_table_size_) {
        throw cRuntimeError("Received BloomFilter cannot be decoded!");
    }
    bit_table_ = table;
}

BloomParameters BloomFilter::getParameters(unsigned int projected_element_count, double false_positive_probability)
{
    BloomParameters opt;
    ...
    if (!opt) {
        throw cRuntimeError("Bloom parameters are not correct!");
    }
...
void BloomFilter::appendToName(iName& name) const //TODO modify needed, because of naming operate changed
{
    std::string tag, prefix, tail;
    tag = name.getTag();
    prefix = name.getPrefix();
    tail = name.getTail();
    tail.append(std::to_string(projected_element_count_));
    tail.append(std::to_string((int)(desired_false_positive_probability_ * 1000)));
    tail.append(bit_table_.begin(), bit_table_.end());

    name.set(tag, prefix, tail);
}
...
std::vector <BloomFilter::cell_type> BloomFilter::table() const { return bit_table_; }
