    ...
    if (pit->haveThisInterest(headInterest))
    {
        ...
        for (auto iter = range.first; iter != range.second; iter++)
        {
            ...
            if (!ct->hasThisPacket(packet, headInterest))
            {
                ...
                if (block == nullptr)
                {
                    block = ct->createBlock(headInterest);
                }
...
shared_ptr<iFib> NFD::findRoute(const iName& nid) //TODO
{
...
void NFD::cachePacket(const iName &interest, Packet *packet)
{
    ct->cachePacket(interest, packet);
}
...
void NFD::handlePacketFromHL(Packet *packet)
{
...
void NFD::fragmentAndStore(Packet *packet, const iName &interest)
{
    const auto &data = makeShared<iData>();
    ...
    if (headerLength + payloadLength < mtu)
    {
        ...
        cachePacket(interest, packet->dup());
    }
    else
    {
...
void NFD::fragmentAndSend(Packet *packet)
{
}
...
void NFD::testProvide(const iName &interest, const B &dataSize)
{
    ...
    fragmentAndStore(packet, interest);
}
void NFD::cacheData(const iName &interest, Packet *packet)
{
    fragmentAndStore(packet, interest);
}
...
//    std::cout<<node<<endl;
    ...
    else if(ndnSocketCacheDataCommand *command = dynamic_cast<ndnSocketCacheDataCommand *>(ctrl))
    {
        ...
        cacheData(command->getInterest(), pkt);
        delete request;
    }
}

void NFD::sendUp(Packet *pkt, SocketDescriptor *sd)
{
...
INetfilter::IHook::Result NFD::datagramPreRoutingHook(Packet *datagram)
{
    return INetfilter::IHook::ACCEPT;
}


INetfilter::IHook::Result NFD::datagramForwardHook(Packet *datagram)
{
    return INetfilter::IHook::ACCEPT;
}


INetfilter::IHook::Result NFD::datagramPostRoutingHook(Packet *datagram)
{
    return INetfilter::IHook::ACCEPT;
}


INetfilter::IHook::Result NFD::datagramLocalInHook(Packet *datagram)
{
    return INetfilter::IHook::ACCEPT;
}


INetfilter::IHook::Result NFD::datagramLocalOutHook(Packet *datagram)
{
    return INetfilter::IHook::ACCEPT;
}

void NFD::registerHook(int priority, IHook *hook)
{

}


void NFD::unregisterHook(IHook *hook)
{

}


void NFD::dropQueuedDatagram(const Packet *datagram)
{

}


void NFD::reinjectQueuedDatagram(const Packet *datagram)
{
